...
data=$1
numsplit=$2 # the number of parties should be the same as the number of session
...
# 这里我应该自己写一个脚本代替，目的是按session划分utt2spk
# 4 表示我们将划分为4个party而不是每一session一个party
# 8 表示我们将划分为8个party，每两个session一个party，同一个session里面，有相同的SPK
python utt2spk_by_session.py 8 $data/utt2spk $utt2spks || exit 1
